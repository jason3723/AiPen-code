...
3. **严格按顺序执行**：检查 → 构建 → 验证 → 更新 latest.json
4. **每步验证结果**：不要假设上一步成功，必须检查产物
5. **签名验证三要素**：产物存在（文件）→ 字节有效（>0）→ 时间戳一致（sig 由本次构建生成）
6. **遇到错误先查第 6 节**：按固定处理方式操作，不要自行发挥
7. **对比时间戳再排查**：`.sig` 修改时间远早于 `.exe` → 配置问题；时间一致 → 密码问题。**不要混淆文件加密与密钥密码锁**
8. **不要跳过签名验证**：`.sig` 文件必须非空
9. **发布前的 Git 操作必须用户确认**：不自动执行 `git push`
